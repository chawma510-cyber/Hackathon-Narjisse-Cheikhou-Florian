...
namespace unity4dv
{
    ...
    public class Plugin4DSEditor : Editor
    {
        ...
        private void BuildFilesInspector(Plugin4DS myTarget)
        {
            ...
            myTarget.SourceType = (SourceType)EditorGUILayout.EnumPopup(new GUIContent(" Data Source", "Choose the source of the data between local file and over http network") , myTarget.SourceType);

            Rect rect = EditorGUILayout.BeginVertical(); ;
            if (myTarget.SourceType == SourceType.Local)
            {
                ...
                myTarget.dataInStreamingAssets = EditorGUILayout.Toggle(new GUIContent("In Streaming Assets","checked if the 4ds file is located in the StreamingAssets folder (necessary to deploy it with the app)"), myTarget.dataInStreamingAssets);
                if (!myTarget.dataInStreamingAssets)
            ...
            else //NETWORK
            {
                ...
                if (Event.current.keyCode == KeyCode.Return)
                {
                    ...
                    myTarget.Close();
                    myTarget.Preview();
                    myTarget.LastPreviewTime = System.DateTime.Now;
                }
            }

            string key = EditorGUILayout.TextField(new GUIContent("Decryption Key", "Decryption key word in case the file has been encrypted"), myTarget.DecryptionKey);
            if (key != myTarget.DecryptionKey) {
                myTarget.DecryptionKey = key;
                myTarget.Close();
                myTarget.Preview();
            }
            ...
            myTarget.debugInfo = EditorGUILayout.Toggle(new GUIContent("Debug Info", "Display some information useful for debugging") , myTarget.debugInfo);
                ...
                case EventType.DragPerform:
                    ...
                    if (evt.type == EventType.DragPerform)
                    {
                        foreach (string path in DragAndDrop.paths)
                        {
                            ...
                            {
                                myTarget.dataInStreamingAssets = true;
                            ...
                            else
                            {
                                ...
                                myTarget.dataInStreamingAssets = false;
                                myTarget.SequenceDataPath = dataPath;
                            }

                            myTarget.SequenceName = seqName;
                            myTarget.SourceType = SourceType.Local;
                            myTarget.PreviewFrame = 0;
                            myTarget.FirstActiveFrame = 0;
                            myTarget.LastActiveFrame = -1;
